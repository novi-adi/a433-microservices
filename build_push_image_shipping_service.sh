...
if command -v docker &> /dev/null; then
    ...
    docker build -t novi-adi/shipping-service:latest .

    # Melihat daftar image di lokal
    docker images

    # Nama paket Docker yang akan digunakan di GitHub Packages
    DOCKER_PACKAGE="ghcr.io/novi-adi/shipping-service:latest"

    # Mengubah nama image agar sesuai dengan format GitHub Packages
    docker tag novi-adi/shipping-service:latest $DOCKER_PACKAGE
